...
function App() {
  ...
  return (
    <>
      <div
        className="min-h-screen flex flex-col items-center justify-center gap-3 sm:gap-5 bg-gradient-to-r from-pink-500 via-red-500 to-yellow-500 p-4"
      >
        <div className="flex flex-col items-center justify-center text-center backdrop-blur-xl bg-black/70 p-3 sm:p-5 rounded-xl">
          ...
          <div className="mb-3 sm:basis-5 text-white text-lg sm:text-xl md:text-2xl">
            <p>Score: {score}</p>
            <p>Best Score: {bestScore}</p>
          </div>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-4 w-full max-w-full p-14">
          {pokemon.map((p) => (
            <div
            ...
            >
              <img
                src={p.image}
                alt={p.name}
                className="w-26 h-16 sm:w-24 sm:h-24 md:w-32 md:h-32 mx-auto"
              />
              <p className="text-center mt-1 sm:mt-2 text-sm sm:text-base">{p.name}</p>
            </div>
          ))}
        </div>
      </div>
